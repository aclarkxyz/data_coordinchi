...
export class ExportContent
{
	...
	private checkRoundTrip(mol:Molecule, src:string):void
	{
		...
		let compareMols = (mol1:Molecule, mol2:Molecule):boolean =>
		{
			...
			for (let n = 1; n <= na; n++)
			{
				if (mol1.atomElement(n) != mol2.atomElement(n)) return false;
				if (mol1.atomCharge(n) != mol2.atomCharge(n) || mol1.atomUnpaired(n) != mol2.atomUnpaired(n)) return false;
				if (mol1.atomHExplicit(n) != mol2.atomHExplicit(n) || mol1.atomIsotope(n) != mol2.atomIsotope(n) || mol1.atomMapNum(n) != mol2.atomMapNum(n)) return false;
				...
				if (!Vec.equals(tx1, tx2) || !Vec.equals(ty1, ty2)) return false;
			}
